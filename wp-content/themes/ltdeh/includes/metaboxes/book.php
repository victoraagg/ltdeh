...
function ltdeh_custom_metabox_book_data_callback($post, $data)
{
    ...
    $_Ds_MerchantParameters = isset($postMeta['_Ds_MerchantParameters']) ? $postMeta['_Ds_MerchantParameters'][0] : '';

    echo '<p>Estado</p>';
    echo '<select name="_book_active">';
    if (!$_book_active || $_book_active == 'N') {
        $selected = 'selected';
    } else {
        $selected = '';
    }
    ...
    if (!$_book_recurrence || $_book_recurrence == 'N') {
        $selected = 'selected';
    } else {
        $selected = '';
    }
